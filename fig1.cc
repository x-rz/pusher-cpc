...
//==============constants===============
#ifdef a_txt
double dt asm("dt")=1.0e-7;
#endif
#ifdef b_txt
double dt asm("dt")=1.0e-5;
#endif
#ifdef c_txt
double dt asm("dt")=1.0e-3;
#endif
#ifdef d_txt
double dt asm("dt")=1.0e-5;
#endif
...
static Mat3 IB_inv;
//============functions=================
inline void FG(){					//calculate electromagnetic tensor F
...
inline void output(){
	printf("%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n",t,u0.x[3],u0.x[0]/u0.x[3],u0.x[1]/u0.x[3],u0.x[2]/u0.x[3],p.x[0]*c_0,p.x[1]*c_0,p.x[2]*c_0);
	fprintf(stderr,"t:%.4e\tgm:%.4e\tp0:%.4e\tp1:%.4e\tp2:%.4e\n",t,u0.x[3],p.x[0]*c_0,p.x[1]*c_0,p.x[2]*c_0);
}
...
void Comp_tau(){
	...
	double BB=B*B,BE=B*E,BU=B**((Vec3*)&u0);coef0=2.*(BE)*(BU)+(BE)*(BE)*dt,coef1=2.*(*((Vec3*)&u0)*(B.CPM()*E)+BB*u0.x[3]),coef2=2.*E**((Vec3*)&u0)+E*E*dt-BB*dt,coef3=2.*u0.x[3],coef4=-dt;
	//Tau_Max();
	double EE=E*E,TEMP0=(BB-EE)*(BB-EE),TEMP1=BE*BE,Res;
	...
	else Res=dt/(1+u0.x[3]);
	if(Res<dt/(1+u0.x[3])) tau_max=Res;else tau_max=dt/(1+u0.x[3]);					//When EE<=BB, find out tau_max(the upper limit of \tau)
	...
	if(fabs(coef0)>1e-100){
		...
		double t1=-coef2/(3.*coef1);
		if(tau_min<t1&&t1<tau_max){if(((4.*coef0*t1+3.*coef1)*t1+2.*coef2)*t1+coef3>0) tau_max=t1; else tau_min=t1;}
	}
	//Tau_Newt();
//inline void Tau_Newt(){		//Newton iteration to solve \tau
	double a=D_qf(tau_min),b=D_qf(tau_max),dt;
...
inline void Update_imp2(){				//algorithm(implementation 2)	
	u1=u0;
	Comp_ib_inv();
	U=IB_inv*(2.*U+E*dt)-U;
...
#endif
#if(defined(b_txt)||defined(c_txt)||defined(d_txt))
	//	Comp_tau();Update_imp1();		//implementation 1
	//	Comp_tau();Update_imp2();		//implementation 2
		Asm_opt();				//optimized
#endif
	}
