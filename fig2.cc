...
inline void field(Vec3 &E,Vec3 &B,const double t){	//Electromagnetic fields.
	E.x[0]=0; E.x[1]=0; E.x[2]=1*COE0; B.x[0]=0; B.x[1]=0; B.x[2]=0;
#ifdef d_txt
	B.x[2]=-3*COE0;
#endif
}
...
int main(int argc, char *argv[]){
	U.x[0]=u0.x[0]=0e3;U.x[1]=u0.x[1]=1e3;U.x[2]=u0.x[2]=0e3;u0.x[3]=gm=sqrt(1+u0.x[0]*u0.x[0]+u0.x[1]*u0.x[1]+u0.x[2]*u0.x[2]);	//initialize particle parameters
	for(t=0;t<1e-4;t+=dt){
