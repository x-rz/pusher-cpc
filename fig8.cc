...
int main(int argc, char *argv[]){
	...
	Lorentz_matrix(L_U);
	double pha,pha0=0.,pha_ana,d_pha;
	for(dt=1e-8;dt<1.1e-3;dt+=1e-6){
		...
		u0=L_M_t*u0;				//transform to computational frame
		VecN U0=u0;
		U.x[0]=u0.x[0];U.x[1]=u0.x[1];U.x[2]=u0.x[2];gm=u0.x[3];
		anal_solu();
		U0=L_M*U0;u0=L_M*u0;			//transform to observational frame
		pha_ana=phase(U0,u0);
		U0=L_M_t*U0;u0=L_M_t*u0;		//back to computational frame
		u0=U0;
