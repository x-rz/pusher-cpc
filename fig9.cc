...
double __attribute__((aligned(32))) data[12] asm("dta")={0,0,0,0,0,0,-3*COE0,0,0,sqrt(1000.*1000.-1.),0,1000.};
//===========static variables===========
volatile double vlt;					//variables used to ensure the compiler does not ignore some code due to optimization
...
static Mat3 IB_inv;
//============functions=================
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void FG(){					//calculate electromagnetic tensor F
...
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void field(Vec3 &E,Vec3 &B,const double t){
	E.x[0]=data[0]+1e-2*COE0*t;E.x[1]=data[1]+1e-2*COE0*t;E.x[2]=data[2]+1e-2*COE0*t;B.x[0]=data[4]+1e-2*COE0*t;B.x[1]=data[5]+1e-2*COE0*t;B.x[2]=data[6]+1e-2*COE0*t;
}
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Comp_coe(){					//compute coefficients
	double BB=B*B,BE=B*E,BU=B**((Vec3*)&u0);coef0=2.*(BE)*(BU)+(BE)*(BE)*dt,coef1=2.*(*((Vec3*)&u0)*(B.CPM()*E)+BB*u0.x[3]),coef2=2.*E**((Vec3*)&u0)+E*E*dt-BB*dt,coef3=2.*u0.x[3],coef4=-dt;
}
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) double Q_f(double &tau){	//quartic function
	return (((coef0*tau+coef1)*tau+coef2)*tau+coef3)*tau+coef4;
}
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) double D_qf(double &tau){	//derivative of the quartic function
//	return ((4.*coef0*tau+3.*coef1)*tau+2.*coef2)*tau+coef3;
	
	double tp0=coef0,tp1=coef1,tp2=coef2;
	tp0=tp0+tp0; tp0=tp0+tp0; tp1=tp1+tp1+tp1; tp2=tp2+tp2;
	return ((tp0*tau+tp1)*tau+tp2)*tau+coef3;

}
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Tau_Max(){	//calculate the upper limit of \tau
	tau_min=0;
...
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Ens_conv(){	//ensure that f is a convex function in [\tau_{min},\tau_{max}]
...
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Tau_Newt(){	//Newton iteration to solve \tau
...
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Comp_J(){	//compute augmented Jacobian matrix
...
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Gauss_Jordan(){	//Gauss Jordan elimination with complete pivoting
	const int N=4;
	int J_order[N];
...
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Comp_tau(){
...
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Update_imp1(){
	Comp_J();
	Gauss_Jordan();
	u1=u0;
	u0.x[0]=res[0];u0.x[1]=res[1];u0.x[2]=res[2];u0.x[3]=res[3];
	p+=(u0+u1)*tau_;				//update 4-position
}
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Comp_ib_inv(){
	Vec3 vec=B*tau_;
	IB_inv.x[0][0]=1.+vec.x[0]*vec.x[0];		IB_inv.x[0][1]=vec.x[2]+vec.x[0]*vec.x[1];	IB_inv.x[0][2]=-vec.x[1]+vec.x[0]*vec.x[2];
	IB_inv.x[1][0]=-vec.x[2]+vec.x[0]*vec.x[1];	IB_inv.x[1][1]=1+vec.x[1]*vec.x[1];		IB_inv.x[1][2]=vec.x[0]+vec.x[1]*vec.x[2];
	IB_inv.x[2][0]=vec.x[1]+vec.x[0]*vec.x[2];	IB_inv.x[2][1]=-vec.x[0]+vec.x[1]*vec.x[2];	IB_inv.x[2][2]=1.+vec.x[2]*vec.x[2];
	IB_inv=IB_inv*(1./(1.+vec*vec));
}
__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Update_imp2(){
	u1=u0;
	Comp_ib_inv();
	U=IB_inv*(2.*U+E*dt)-U;
...
//__inline __attribute__((__gnu_inline__, __always_inline__, __artificial__)) void Asm_opt(){
void Asm_opt(){
	field(E,B,t+0.5*dt);									//compute field
	asm(
...
int main(int argc, char *argv[]){
	...
	for(int i=0;i<sn;i++){
		p.x[0]=0,p.x[1]=0,p.x[2]=0;p.x[3]=0;
		stt=clock();
#ifdef imp1		
		for(t=0;t<=1e2;t+=dt)
		{
			U.x[0]=u0.x[0]=data[8];U.x[1]=u0.x[1]=data[9];U.x[2]=u0.x[2]=data[10];gm=u0.x[3]=data[11];		//initialize particle parameters
			Comp_tau();Update_imp1();
		}		
#endif
#ifdef imp2
		for(t=0;t<=1e2;t+=dt)
		...
		vlt=p.x[0];vlt=p.x[1];vlt=p.x[2];			//vlt1 used to ensure the compiler does not ignore related code
	}
